...
export const Navbar = () => {
    ...
    return (
        <nav className="navbar w-full bg-nav px-5 py-4 flex justify-between items-center sm:px-10 sm:py-6">
            <div className="logo flex gap-4 items-center">
                <img src="img/logo.png" className="h-10 sm:h-12"></img>
                <h1 className="font-bold text-white text-3xl sm:text-4xl">Szymon</h1>
            </div>

            <div className={Toggle ? "links text-text shadow-md gap-8 translate-y-96 sm:translate-y-0 items-center grid bg-white rounded-lg px-3 fixed py-8 sm:flex-row sm:gap-4 sm:relative bottom-12 grid-cols-2 justify-center text-md sm:bg-nav sm:text-lg sm:top-0 sm:m-0 sm:px-0 sm:py-0 sm:shadow-none sm:flex ease-in duration-200" : "links text-text shadow-md translate-y-0 ease-in duration-200 gap-8 items-center bg-white rounded-lg px-3 py-8 sm:flex-row sm:gap-4 sm:relative bottom-12 fixed grid grid-cols-2 justify-center text-md sm:bg-nav sm:text-lg sm:top-0 sm:m-0 sm:px-0 sm:py-0 sm:flex sm:shadow-none"}>
                ...
                <a href="https://github.com/lis3q" target="_blank" className="text-md py-2 px-5 text-primary rounded-lg border-2 border-primary hover:text-white hover:bg-primary ease duration-200 block sm:hidden">Mój GitHub</a>
            </div>
            ...
            <div onClick={() => showMenu(!Toggle)} className="hamburger-menu block sm:hidden">
